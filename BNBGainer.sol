...
contract BnbGainers {
    ...
    function addInvestment(address _userAddress,address _sponsorAddress, uint _investedAmount,uint _timestemp) external noReentry payable{
        //Basic validations
        ...
        // require(users[_sponsorAddress].is_active == false, "The given sponsor not registered!");
        ...
        if(users[_userAddress].is_active == true){
            ...
            UserStruct memory newUserObject = generateOldUser(_userAddress, _sponsorAddress, _investedAmount, _chainName, _timestemp);
    ...
    function withdraw(uint _withdrawAmountRequested) external noReentry payable{
        ...
        require(block.timestamp >= (withdrawingUser.last_withdraw_time  ), "You can only withdraw once every 24 hours!");
